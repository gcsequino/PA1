...
public class OneB extends Configured implements Tool{
        ...
        throws IOException, InterruptedException{
            ...
            for(int i = 0; i < tokens.length; i++){
                if(tokens[i].length() >= 1){
                    kv_pair = tokens[i] + "\t" + author[date.length -1];
                    context.write(new Text(kv_pair), new Text(book_id));
                }
            }
    ...
    public static void main(String[] args) throws Exception { 
		...
		int res = ToolRunner.run(new Configuration(), new OneB(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setMapperClass(OneB.WordCountMapper.class); 
        job.setReducerClass(OneB.WordCountReducer.class);
