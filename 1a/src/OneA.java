...
public class OneA extends Configured implements Tool{
        ...
        throws IOException, InterruptedException{
            ...
            for(int i = 0; i < tokens.length; i++){
                if(tokens[i].length() >= 1){
                    kv_pair = tokens[i] + "\t" + date[date.length -1];
    ...
    public static void main(String[] args) throws Exception { 
		...
		int res = ToolRunner.run(new Configuration(), new OneA(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setMapperClass(OneA.WordCountMapper.class); 
        job.setReducerClass(OneA.WordCountReducer.class);
