...
public class TwoA extends Configured implements Tool{
            ...
            for(int i = 0; i <= tokens.length; i++) {
                if(i == 0) { // start of the line
                    if(tokens[i].length() >= 1){
                        kv_pairs = "_START_" + "," + tokens[i] + "\t" + date[date.length-1];
                        context.write(new Text(kv_pairs), new Text(book_id));
                    }
                }
                else if(i == tokens.length) { // end of the line
                    if(tokens[i-1].length() >= 1) {
						kv_pairs = tokens[i-1] + "," + "_END_" + "\t" + date[date.length -1];
						context.write(new Text(kv_pairs), new Text(book_id));
					}
                }
                else { // middle of the line
                    if(tokens[i].length() >= 1){
						kv_pairs = tokens[i-1] + "," + tokens[i] + "\t" + date[date.length -1];
						context.write(new Text(kv_pairs), new Text(book_id));
					}
                }
            }
    ...
    public static void main(String[] args) throws Exception { 
		...
		int res = ToolRunner.run(new Configuration(), new TwoA(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setMapperClass(TwoA.WordCountMapper.class); 
        job.setReducerClass(TwoA.WordCountReducer.class);
